// mcp-output-guard.ts - Bound model-facing MCP output.
//
// Ported verbatim from pi-mcp-adapter. Inline text is capped at 50 KiB /
// 2000 lines; oversized output is spilled to a temp file whose path is
// included in the result. Image blocks pass through untouched. The raw
// `details.mcpResult` is kept when ≤ 16 KiB; larger results are replaced
// with a compact summary and spilled to a temp file.
//
// Kill switch: `MCP_OUTPUT_GUARD=0` env var or `settings.outputGuard = false`.

import { randomBytes } from "node:crypto";
import { mkdtemp, writeFile } from "node:fs/promises";
import { tmpdir } from "node:os";
import { join } from "node:path";
import type { ContentBlock, BridgeSettings } from "./types.ts";
...
export interface McpOutputGuardOptions {
  ...
  /** Raw MCP result to expose as details.mcpResult. */
...
/** Exported for `config.ts` to honor the env kill switch. */
export function envKillSwitch(name: string): boolean | undefined {
...
async function saveArtifact(kind: string, text: string): Promise<{ path?: string; error?: string }> {
  try {
    const dir = await mkdtemp(join(tmpdir(), "pi-mcp-bridge-"));
