...
//
// Ported verbatim from pi-mcp-adapter. When a server is configured with
// `command: "npx"`, the bridge resolves the underlying binary directly
// from the npm cache so we don't pay the ~143 MB npm parent process on
// every call. Falls back to the original `npx` command if resolution
// fails.
