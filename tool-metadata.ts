// tool-metadata.ts - ToolMetadata helpers.
//
// Ported from pi-mcp-adapter. Used by the wrapper tools to look up the
...
/** List the (prefixed) tool names for a server. */
export function getToolNames(
  toolMetadata: Map<string, ToolMetadata[]>,
  serverName: string,
): string[] {
  return toolMetadata.get(serverName)?.map(m => m.name) ?? [];
}

/** Find a tool by name, with fuzzy matching on hyphens/underscores. */
...
/** Pretty-print a JSON Schema as indented parameter list (for error messages). */
...
function s_required(value: unknown): string[] {
