// ui-stream-types.ts - UI stream envelope schemas (lightweight, no pi-coding-agent dep).
//
// Ported from pi-mcp-adapter with the adapter-owned meta keys renamed
// from `pi-mcp-adapter/*` to `pi-mcp-bridge/*`. This module is shared
// with the example package so it stays free of the full types.ts
// dependency graph.

import { z } from "zod";

export const UI_STREAM_HOST_CONTEXT_KEY = "pi-mcp-bridge/stream";
export const UI_STREAM_REQUEST_META_KEY = "pi-mcp-bridge/stream-token";
export const UI_STREAM_RESULT_PATCH_METHOD = "notifications/pi-mcp-bridge/ui-result-patch";
export const SERVER_STREAM_RESULT_PATCH_METHOD = "notifications/pi-mcp-bridge/result-patch";
export const UI_STREAM_STRUCTURED_CONTENT_KEY = "pi-mcp-bridge/stream";
