 ...
 *
 * A failed MCP tool call is *returned* (not thrown), tagged
 * `details.error: "tool_error"` (the server returned an error result) or
 * `"call_failed"` (the call itself threw and was caught). Pi never reads a
 ...
 * intact.
 *
 * Limited to those two codes: the bridge's other `details.error` values
 * (`auth_required`, connection states, validation feedback, ...) are not
 * failed tool calls, so they get no override.
 */
