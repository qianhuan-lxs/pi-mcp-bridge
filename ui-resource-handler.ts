// ui-resource-handler.ts - Fetch + parse MCP UI resources.
//
// Ported from pi-mcp-adapter. When a tool's registry descriptor
// declares `ui.resourceUri`, the bridge fetches the HTML resource from
// the MCP server, validates its MIME type, extracts CSP/permissions
// metadata, and hands the HTML to the UI session for rendering.
