// metadata-cache.ts - Persistent MCP metadata cache (compat layer).
//
// In pi-mcp-adapter this cache was the primary source of truth for
// tool metadata. In pi-mcp-bridge the filesystem registry is the source
// of truth. We keep this module for two reasons:
//   1. Fast reconnect: when a server's tools haven't changed, we can
//      skip `tools/list` after reconnect by comparing config hashes.
//   2. npx resolution cache lives next to it under the agent dir.
//
// The cache is NOT read by the context injector (the registry is).
...
export function computeServerHash(definition: ServerEntry): string {
  ...
  // Exclude lifecycle, idleTimeout, requestTimeoutMs, debug — those are
  // runtime behavior settings that don't change which tools a server exposes.
